...
namespace ValheimArmory.Common {
    ...
    internal static class ItemConfigDrawer {
        ...
        private static readonly (string name, ItemStat[] stats)[] StatGroups = new (string, ItemStat[])[] {
            ("Damage", new[] {
        ...
        private static void Draw(ItemDefinition itemdef) {
            ...
            if (expanded) {
                ...
                foreach ((string name, ItemStat[] stats) group in StatGroups) {
                    List<ItemStat> present = group.stats.Where(s => IsConfigurable(itemdef, s)).ToList();
                    bool damageMods = group.name == "Damage" && itemdef.DamageMods != null && itemdef.DamageMods.Count > 0;
                    if (present.Count == 0 && !damageMods) { continue; }
                    ConfigDrawHelpers.GroupHeader(group.name);
        ...
        internal static string SerializeCsv(List<string[]> rows) =>
            string.Join("|", rows.Where(r => !string.IsNullOrWhiteSpace(r[0])).Select(r => string.Join(",", r)));
